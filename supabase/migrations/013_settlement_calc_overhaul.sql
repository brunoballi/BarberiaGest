-- ============================================================
-- LIQUIDACIONES — Revisión del cálculo (calculate_settlement)
-- Cambios respecto de la versión original:
--  1. objetivo_met pasa a ser OVERRIDE MANUAL del admin (igual que
--     presentismo_met): se pre-llena con la sugerencia automática (umbral de
--     cortes) en el primer cálculo y luego se preserva con coalesce.
--  2. Los bonos de objetivo/presentismo aplican a 'salary' Y 'percentage'
--     (no a 'box_rental'); se calculan como % del total facturado.
--  3. NO se generan liquidaciones vacías: si el barbero no tiene cortes en la
--     semana, no se crea liquidación y se elimina el borrador vacío si quedó
--     de un recálculo previo (no se tocan confirmed/paid).
