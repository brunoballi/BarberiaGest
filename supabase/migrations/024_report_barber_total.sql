-- ============================================================
-- REPORTE: Total Barberos (con bonos) + desglose por barbero
-- ------------------------------------------------------------
-- "Total Barberos" = lo que se lleva cada barbero =
--    Σ barber_share (comisión por corte)  +  Σ bonos (presentismo + objetivo)
-- "Total Barbería" = Ingresos − Total Barberos  (se calcula en el cliente)
--
-- Antes "Comisiones" = Σ barber_share (sin bonos) y "Parte negocio" =
-- Σ branch_share (sin descontar bonos). Ahora los bonos se atribuyen al
-- barbero y se descuentan de la barbería.
--
-- Devuelve además `barbers`: desglose [{barber_id, full_name, total}] para
-- el desplegable.
-- ============================================================
...
as $function$
  ...
  -- Comisión por corte de cada barbero (transacciones del período)
  barber_tx as (
    select branch_id, barber_id, coalesce(sum(barber_share),0)::float8 as bshare
    from transactions
    where branch_id = any(p_branch_ids)
      and transaction_date between p_start and p_end
    group by branch_id, barber_id
  ),
  -- Bonos de cada barbero (liquidaciones de las semanas del período)
  barber_bonus as (
    ...
    where s.branch_id = any(p_branch_ids)
      and w.start_date between p_start and p_end
