-- 027: el efectivo de cortes VIP (full_amount_to_barber, barbero a comisión %)
-- NO cuenta como efectivo en caja en la liquidación: el barbero se lo queda en
-- el momento. settlements.cash_amount pasa a significar "efectivo que entró a
-- la caja de la barbería" (alimenta las tarjetas Efectivo del panel semanal).
-- Único cambio vs. la versión anterior: el sum(cash_amount) del bloque de
-- agregación excluye esos cortes (left join benefits).
