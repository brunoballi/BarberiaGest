-- 038: el beneficio VIP funciona igual para el barbero nuevo que para el resto.
--
-- Bug 1 — calculate_settlement: en el barbero nuevo, el reparto por tramos se
-- calcula sobre el neto del día (que EXCLUYE el VIP), pero nunca se le sumaba
-- el VIP que el barbero se lleva al 100%. Resultado: total_earned quedaba corto
-- (el VIP faltaba del ganado pero sí estaba en already_collected), el neto a
-- pagar daba de menos, y la grilla mostraba "Comisión base" negativa (restaba
-- vip_amount de una comisión que ya lo excluía).
-- Fix: sumar v_vip_amount a barber_comision (y por ende a barber_gross), igual
-- que en el barbero % común, donde el VIP viaja dentro de barber_share.
--
-- Ejemplo (clásico 12.500 -> básico 25.000; 1 corte VIP de 12.500 + neto 25.000):
--   tramo día = 25.000 (<= básico)  +  VIP 12.500  =>  total ganado 37.500
--   ya cobrado (VIP en mano) 12.500  =>  a pagar 25.000  (antes daba 12.500)
--
-- Bug 2 — recalculate_settlement_full: la versión de la 034 perdió el fix VIP
-- de prod (commit cae947f): al recalcular, pisaba el barber_share de los cortes
-- VIP con la fórmula de comisión (revirtiendo el 100%/0 al reparto normal).
-- Fix: en cortes VIP, barber_share = amount y branch_share = 0, manteniendo la
-- fórmula con descuento de la 034 para el resto de los cortes.
      ...
      select
        ...
        case
          when coalesce(bf.full_amount_to_barber, false) then tx.amount
            ...
            round(
              (tx.amount + coalesce(tx.discount_amount, 0)) * coalesce(v_barber.commission_rate, 0)
              - coalesce(tx.discount_amount, 0) * 0.5
            , 2),
            tx.amount
          ))
        end as bshare
