-- ============================================================
-- ALQUILER DE BOX EN LA LIQUIDACIÓN (settlements.box_rent)
-- En box_rental el barbero se queda el 100% de los cortes; la barbería cobra
-- el alquiler del box, que el admin carga manualmente en la liquidación (borrador).
-- box_rent se descuenta del neto (lo paga el barbero a la barbería) y se preserva
-- entre recálculos. Reemplaza calculate_settlement (mantiene reglas previas:
-- objetivo override manual, bonos % comisión, no generar vacías, box_rental 100%).
-- security definer. Idempotente.
-- ============================================================

alter table settlements add column if not exists box_rent numeric not null default 0;
comment on column settlements.box_rent is
  'Alquiler de box que el barbero (box_rental) le paga a la barberia esa semana. Editable en borrador; se descuenta en net_payable.';
...
begin
  ...
  insert into settlements (
    ...
    objetivo_met, presentismo_met, box_rent, status, updated_at
  ) values (
    ...
    v_objetivo_met, v_presentismo_met, v_box_rent, 'draft', now()
  )
